...
sql0="update arrange_info set state='录入完成' where arrange_no='"&arrange_no&"'"
...
<table width="100%" cellspacing=1>
  ...
  %>
  <tr>
  	<th>通知单号：</th><td align=center><%=notify_no%></td><th>款号：</th><td align=center><%=design_no%></td><th>排产人：</th><td align=center><%=rs8("lister")%></td><th>排产日期：</th><td align=center><%=rs8("date_created")%></td>
  </tr>
</table>
<table width="100%" cellspacing=1>
	<tr>
  ...
  %>
    ...
    fact_workload=(this_arrange_amount_sum*man_hour)/(datediff("d",rs("date_start"),rs("date_finish"))+1)/yesterday_all_hour
  	...
  	<th>
  	<input name="input_order" type="button" onclick="MM_goToURL('self','arrange_finish.asp?arrange_no=<%=arrange_no%>&notify_no=<%=notify_no%>&design_no=<%=design_no%>&arrange_group=<%=rs("arrange_group")%>');return document.MM_returnValue" value="查看详细">
  </th></tr>
  <%
    rs.movenext
  wend
...
<br>
<center>
  <input name="input_order" type="button" onclick="MM_goToURL('self','list_arrange.asp');return document.MM_returnValue" value="确 定">
</center>
</body>
</html>