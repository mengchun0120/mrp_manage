...
<html>
  ...
  <body topmargin=0 leftmargin=0>
	...
	<tr>
  	<td class="table_title" colspan=6>生产通知单：“<%=notify_no%>” 生产小组：“<%=arrange_group%>” 排产</td>
  </tr>
  <form action="analyze_workload.asp" method=post target="mainFrame">
  <tr>
  	<th>开始日期：</th><td align=center><input type="text" name="date_start" size=10 value="<%=date_start%>">（格式：YYYY-MM-DD）</td>
  	<th>清批日期：</th><td align=center><input type="text" name="date_finish" size=10 value="<%=date_finish%>">（格式：YYYY-MM-DD）</td>
  	<th>每日工作时长：</th><td align=center><input type="text" name="work_time" size=2 value="<%=work_time%>"> 小时（格式：数字）</td>
  </tr>
</table>
<%
...
<input name="input_order" type="button" onclick="MM_goToURL('self','list_group.asp?arrange_no=<%=arrange_no%>&notify_no=<%=notify_no%>&design_no=<%=design_no%>');return document.MM_returnValue" value="返 回">
</form>
<br><br>
</center>
  </body>
