...
<table width="100%" cellspacing=1>
  ...
  <tr>
  	...
  	%>
  	    ...
  	    <th>
  	    	<input name="input_arrange" type="button" onclick="MM_goToURL('self','is_stop_product.asp?arrange_no=<%=rs4("arrange_no")%>&arrange_group=<%=rs("group_name")%>');return document.MM_returnValue" value="停产">
  	    	<input name="input_arrange" type="button" onclick="MM_goToURL('self','input_date_finish.asp?arrange_no=<%=rs4("arrange_no")%>&arrange_group=<%=rs("group_name")%>');return document.MM_returnValue" value="延期">
  	    	<input name="input_arrange" type="button" onclick="MM_goToURL('self','edit_arrange_amount.asp?arrange_no=<%=rs4("arrange_no")%>&arrange_group=<%=rs("group_name")%>&notify_no=<%=rs4("notify_no")%>&design_no=<%=rs4("design_no")%>');return document.MM_returnValue" value="增加或减少">
  	    </th>
  	    </tr>
  	<%
