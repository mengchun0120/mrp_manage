...
if group_name="" then
	...
	return_url="input_group.asp"
	time_out=2
else
	set rs=Server.CreateObject("ADODB.Recordset")
	sql="select * from group_info where group_name='"&group_name&"'"
	...
	if not rs.eof then
		...
		time_out=2
	  	...
	  	sql2="insert into group_info (group_name) values ('"&group_name&"')"
	  	rs2.open sql2,conn,1,3
	 	rs2.close
    		set rs2=nothing
  		errmsg=group_name&"的信息录入成功！"
		return_url="input_group.asp"
		time_out=0
	end if
	rs.close
