...
<table>
	<tr>
  	<td class="table_title" colspan="4">修改单耗中的面料</td>
  </tr>
<%
...
if (caozuo="更新") then
	...
	cj=request.form("cj")
	bz=request.form("bz")
  insql="update unitconsume_out set consume_name='"&mc&"',consume_color='"&ys&"',consume_chengf='"&cf&"',consume_fuk='"&fk&"',consume_danw='"&dw&"',consume_kegyl="&kg&",consume_shijyl="&sj&",consume_sunh="&sh&",consume_caijyl="&cj&",consume_beiz='"&bz&"' where consume_id="&conid&""
  conn.execute(insql)
  response.redirect "ifc_dhjs.asp?conid="&order
end if
xgsql="select * from unitconsume_out where consume_id="&conid&""
set rsxg=conn.execute(xgsql)
if not rsxg.eof then%>
<form name=myform method=post action="out_update_dhjs.asp">
